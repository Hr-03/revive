...
function PatientFollowUpDetails(){
...
let PatId=sessionStorage.getItem("PatId");
    ...
    const handleSubmit=(e)=>{
      ...
      const addfUrl=`https://reviveapplication.com/ReviveAPI/Revive.svc/AddNewJoinedFollowup`;


      if((fup?.FollowUpStatus==="2" && fup?.NextFollowUpDate==="") || fup?.FollowUpStatus==="" || fup?.ConversationDetails==="" || fup?.Rating==="" || fup?.EnquiryFor==="" || fup?.AssignedToUser==="" || fup?.FollowUpMode===""){
        Swal.fire({
          icon:"warning",
...
const AssignedToUserUrl=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetEmployeeDetails/0`;


useEffect(()=>{
fetch(AssignedToUserUrl)
.then((res)=>res.json())
.then((stat)=>{
  console.log(stat.Data);
  setAssignedToUser(stat.Data);
})
},[])

const [enquiryfor, setEnquiryFor] = useState([]);

const EnquiryForUrl=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetTreatmentList`;
...
const patDataUrl=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetPatientFollowupData/${PatId}`;
...
useEffect(()=>{
  ...
  .then((fupd)=>{
    console.log(fupd.Data);
    if(fupd.Data.length != 0 ){
      setFupDetails(fupd.Data);
      setFup((pre)=>{
        
        
        console.log(fupd.Data[0]?.NextFollowUpDate);
        let fullName = fupd.Data[0]?.Name;
...
console.log(fupDetails[0]?.NextFollowUpDate);
        ...
        <Row>
            ...
            <Row>
                ...
                <Card style={{backgroundColor:"#825AA5",color:"white"}} className="p-4">
                    <Row>
                        <Col md={6}>
                        <p style={{color:"#ffb5ca",fontWeight:"600"}}>Name :<span className="px-2" style={{color:"white",fontWeight:"normal"}}>{patientDetail[0]?.Name}</span></p>
                        <p style={{color:"#ffb5ca",fontWeight:"600"}}>Address :<span className="px-2" style={{color:"white",fontWeight:"normal"}}>{patientDetail[0]?.Address}</span></p>
                        <p style={{color:"#ffb5ca",fontWeight:"600"}}>Source :<span className="px-2" style={{color:"white",fontWeight:"normal"}}>{patientDetail[0]?.Source}</span></p>
                        </Col>

                        <Col md={6}>
                        <p style={{color:"#ffb5ca",fontWeight:"600"}}>Mobile Number :<span className="px-2" style={{color:"white",fontWeight:"normal"}}>{patientDetail[0]?.MobileNo}</span></p>
...
//                   )}
        ...
        <Form.Control type="text" placeholder="" name="FirstName" value={patientDetail[0]?.Name.split(/\s+/)[0]} onChange={(e)=>handleChange(e)}/>
        ...
        <Form.Control type="text" placeholder="" name="LastName" value={patientDetail[0]?.Name.split(/\s+/).slice(1).join(' ')} onChange={(e)=>handleChange(e)}/>
       
      </Form.Group>
                    </Col>

                    {fup?.FollowUpStatus === "2"  || fup?.FollowUpStatus === "23"  || fup?.FollowUpStatus === null ? (
  <Col md={3}>
    <Form.Group className="mb-3" controlId="formBasicEmail">
      <Form.Label>
      ...
      <option></option>
      {
        ...
        <Form.Check type="radio" label="Hot" name="Rating" value="Hot" onChange={(e)=>handleChange(e)}/>
                    ...
                    <Col md={3}>
                        <Form.Group className="mb-3" controlId="formBasicEmail">
                            <Form.Label>Assigned To User <span className="req-f">*</span></Form.Label>
                                <Form.Select aria-label="Default select example" name="AssignedToUser" onChange={(e)=>handleChange(e)}>
                                    <option></option>
                                    {
                                        assignedtouser && assignedtouser.map((data)=>{
                                        return(
                                            <>
                                            <option value={data?.UserID} key={data?.UserID}>{data?.Name}</option>
                    ...
                    <Col md={3}>
                        <Form.Group className="mb-3" controlId="formBasicEmail">
                            <Form.Label>Enquiry For<span className="req-f">*</span></Form.Label>
                                <Form.Select aria-label="Default select example" name="EnquiryFor" onChange={(e)=>handleChange(e)}>
                                    <option></option>
                                    {
                                        enquiryfor && enquiryfor.map((data)=>{
                                        return(
                                            <>
                                            <option value={data?.TreatmentID} key={data?.TreatmentID}>{data?.Treatment}</option>

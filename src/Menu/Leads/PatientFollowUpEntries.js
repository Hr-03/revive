...
function PatientFollowUpEntries() {
  ...
  const handleDates=(e)=>{
    ...
    setdatedata(newdata);
    localStorage.setItem('patientfollowupentriesdatedata', JSON.stringify(newdata));
    console.log(newdata);
  }

  // Function to load saved dates from local storage on component mount
  useEffect(() => {
    const savedDates = localStorage.getItem('patientfollowupentriesdatedata');
    if (savedDates) {
...
//   },[])
  ...
  if(localStorage.getItem('patientfollowupentriesdatedata') != null){
    const searchdatedata = JSON.parse(localStorage.getItem('patientfollowupentriesdatedata'));
    // Use the extracted startDate and endDate in the API URL
    fentUrl = `https://reviveapplication.com/ReviveAPI/Revive.svc/GetFollowupListPatient/${searchdatedata?.startDate}/${searchdatedata?.endDate}/${User}`;
  }
  else{
    fentUrl = `https://reviveapplication.com/ReviveAPI/Revive.svc/GetFollowupListPatient/0/0/0`;
  }
  ...
  return (
    ...
    <Card className="m-1 mt-3 fup-crd p-3">
            <Row>
              <Col>
                <p className="ap-t">Patient Follow Up Entries</p>
                <hr />
...
<div className='pt-3'>
      ...
      <Button variant='' className='mx-3 rptBtn mt-4' onClick={(e)=>{
        ...
        const datefiltered=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetFollowupListPatient/0/0/0/${datedata?.startDate}/${datedata?.endDate}/${User}`
        fetch(datefiltered)
        .then((res)=>res.json())
        .then((geteRes)=>{
                    ...
                    <Box sx={{ display: "flex", gap: "1rem" }}>
                      ...
                      {/* <Tooltip arrow placement="right" title="convert">
                        <IconButton
                          ...
                          onClick={() => {
                            let patientId = cell.row.original.PatientID;
                            sessionStorage.setItem("convpatientId", patientId);
                            console.log(cell.row.original.PatientID);

                            navigate(`/fup-pnt/${patientId}`)
                          }}
                        >
                          <HiUserAdd />
                        </IconButton>
                          ...
                          onClick={() => 
                            {
                              ...
                              sessionStorage.setItem("convpatientId", patientId);
                              console.log(cell.row.original.PatientID);
                              navigate(`/patientsfup-view/${patientId}`)}
                            }
                        >
                          <AiOutlineEye />
                        </IconButton>
...
export default PatientFollowUpEntries;

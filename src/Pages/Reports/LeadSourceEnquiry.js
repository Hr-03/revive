...
useEffect(()=>{
  ...
  <Card className="m-1 mt-3 emp-crd p-3">
        <Row>
            <Col>
            <p className="ap-t">Lead Source Enquiries</p>
            <hr />
        <Row className="mt-4">
          <Col>
...
<div className='pt-3'>
      ...
      <Button variant='' className='mx-3 rptBtn mt-4' onClick={(e)=>{
        ...
        const datefiltered=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetLeadSourceEnquiries/${datedata?.startDate}/${datedata?.endDate}/${User}`
        fetch(datefiltered)
        .then((res)=>res.json())
        .then((geteRes)=>{
          console.log(geteRes.Data);
...
export default LeadSourceEnquiry
...
function DoctorwiseCollection() {
  ...
  const [doctorColl, setDoctorColl] = useState([]);

  const getCollUrl=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetDoctorWiseCollectionReport/0/0/0/0/0/${User}`;
useEffect(()=>{
  fetch(getCollUrl)
  .then((res)=>res.json())
  .then((geteRes)=>{
    console.log(geteRes.Data);
    setDoctorColl(geteRes.Data)
  })
},[])
  ...
  <Card className="m-1 mt-3 emp-crd p-3">
        <Row>
            <Col>
            <p className="ap-t">Doctorwise Collection</p>
            <hr />
        <Row className="mt-4">
          <Col>
...
<div className='pt-3'>
      ...
      <Button variant='' className='mx-3 rptBtn mt-4' onClick={(e)=>{
        ...
        const datefiltered=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetDoctorWiseCollectionReport/0/0/0/${datedata?.startDate}/${datedata?.endDate}/${User}`
        fetch(datefiltered)
        .then((res)=>res.json())
        .then((geteRes)=>{
          console.log(geteRes.Data);
...
<div className='d-flex justify-content-between m-2'>
  <CSVLink data={doctorColl} style={{textDecoration:"none",color:"white",backgroundColor:"green",borderRadius:"5px"}} className='p-2'><LiaDownloadSolid fontSize={25}/>Excel</CSVLink>
...
export default DoctorwiseCollection
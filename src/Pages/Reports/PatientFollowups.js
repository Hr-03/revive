...
function PatientFollowups() {
    ...
    const getCollUrl=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetFollowupListPatient/0/0/${User}`;
  useEffect(()=>{
    fetch(getCollUrl)
    .then((res)=>res.json())
    .then((geteRes)=>{
  ...
  return (
    <>
     ...
     <Card className="m-1 mt-3 emp-crd p-3">
        <Row>
            <Col>
            <p className="ap-t">Joined Follow-ups</p>
            <hr />
...
<div className='pt-3'>
      ...
      <Button variant='' className='mx-3 rptBtn mt-4' onClick={(e)=>{
        ...
        const datefiltered=`https://reviveapplication.com/ReviveAPI/Revive.svc/GetFollowupListPatient/${datedata?.startDate}/${datedata?.endDate}/${User}`
        fetch(datefiltered)
        .then((res)=>res.json())
        .then((geteRes)=>{
          console.log(geteRes.Data);
